...
export default {
  ...
  methods: {
    ...
    addToCart: function (product) {
      axios.post('http://127.0.0.1:8004/basket/add/', {product}).then(
          response => {
  ...
  mounted() {
    this.product = null;
    axios.get('http://127.0.0.1:8002/product/' + this.$route.params.slug).then(
        response => {
          this.product = response.data[0];
          console.log(response.data[0]);
          this.breadcrumbs[2].text = response.data[0].name

          axios.get('http://127.0.0.1:8002/opinionProd/?prod_id=' + this.product.catalog_item_id).then(
              response => {
